    ...
    for category in categories:
        ...
        for term_category, jp, en, description, slug in terms:
            if term_category != category:
                continue
            priority = '<span class="term-priority">重点解説</span>' if slug else ""
            detail = f'<a class="term-link" href="glossary/{slug}.html">詳しく見る →</a>' if slug else ""
            rows.append(f'<article class="term-card" data-term="{esc(str(jp))} {esc(str(en))} {esc(category)}"><h3>{esc(str(jp))}</h3><p class="term-en">{esc(str(en))}</p>{priority}<p>{esc(str(description))}</p>{detail}</article>')
