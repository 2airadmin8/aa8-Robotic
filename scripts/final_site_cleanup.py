        ...
        if path.name == "404.html":
            ...
            if path.name == "glossary.html":
                ...
                if hero and tools and hero.start() < tools.start():
                    hero_html, tools_html = hero.group(0), tools.group(0)
                    new = new[:hero.start()] + tools_html + hero_html + new[tools.end():]
        if new != text:
            path.write_text(new, encoding="utf-8")
            updated += 1
        check = new
        if "AIRADMIN8" in check:
            errors.append(f"Uppercase AIRADMIN8 remains: {path.relative_to(output)}")
        if REMOVE_SENTENCE in check:
