(() => {
  ...
  const manufacturerMap = {
    unitree: {
      name: 'Unitree', page: '../manufacturers/unitree.html', makerQuery: 'unitree',
    ...
    agibot: {
      name: 'AgiBot', page: '../manufacturers/agibot.html', makerQuery: 'agibot',
      products: ['agibot-g2', 'agibot-x2-edu'],
      uses: [
    ...
    limx: { name: 'LimX Dynamics', page: '../products/limx-oli.html', products: ['limx-oli'] },
    tianji: { name: 'Tianji', page: '../products/tianji-marvin.html', products: ['tianji-marvin'] },
  };

  const productMaker = {
    'unitree-g1-d': 'unitree', 'unitree-g1': 'unitree', 'unitree-go2-edu': 'unitree',
    'agibot-g2': 'agibot', 'agibot-x2-edu': 'agibot', 'limx-oli': 'limx', 'tianji-marvin': 'tianji',
  };
  ...
  if (productSlug && productMaker[productSlug]) renderProductManufacturer(productSlug, productMaker[productSlug]);
        ...
        <div class="wrap">
          <div class="relation-panel">
            ...
            <div class="relation-actions">
              ${config.resources ? `<a class="button secondary" href="${config.resources}">関連SDK・資料を見る</a>` : ''}
              <a class="button primary" href="../contact.html?maker=${encodeURIComponent(config.name)}">${escapeHtml(config.name)}の導入を相談する</a>
            </div>
          </div>
        </div>`;
  ...
  function renderProductManufacturer(productSlug, makerId) {
    const config = manufacturerMap[makerId];
    ...
    const page = makerId === 'limx' || makerId === 'tianji' ? config.page : config.page;
    block.innerHTML = `
      <p><strong>メーカー情報</strong><br>${escapeHtml(config.name)}の関連製品、SDK、用途、導入条件をまとめて確認できます。</p>
      <a href="${page}">${escapeHtml(config.name)}のメーカー・関連情報を見る →</a>`;
